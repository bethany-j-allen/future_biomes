...
ggplot(data = to_plot_biomes, aes(x = midpoints, y = change_counts,
                                  group = V3, col = V3)) +
    geom_line() +
    xlab("Year") + ylab("Proportion of grid cells undergoing biome transitions between slices") +
    theme_classic()

ggplot(data = to_plot_mega, aes(x = midpoints, y = mega_counts,
                                group = V3, col = V3)) +
  geom_line() +
  xlab("Year") + ylab("Proportion of grid cells undergoing megabiome transitions between slices") +
  theme_classic()
...
for (k in 1:length(RCPs)){
  ...
  for (i in 1:(length(slices)-1)){
    change_counts[i] <- length(which(biomes[,4] != biomes[,i+4])) / (nrow(biomes) - 1)
    mega_counts[i] <- length(which(megabiomes[,4] != megabiomes[,i+4])) / (nrow(biomes) - 1)
...
ggplot(data = to_plot_biomes, aes(x = midpoints, y = change_counts,
                                  group = V3, col = V3)) +
  geom_line() +
  xlab("Year") + ylab("Proportion of grid cells undergoing biome transitions compared to 2000-2020") +
  theme_classic()

ggplot(data = to_plot_mega, aes(x = midpoints, y = mega_counts,
                                group = V3, col = V3)) +
  geom_line() +
  xlab("Year") + ylab("Proportion of grid cells undergoing megabiome transitions compared to 2000-2020") +
  theme_classic()
...
for (j in 1:(length(slices)-1)){
  changes_high[j] <- length(which(high_lat[,4] != high_lat[,j+4])) / (nrow(high_lat) - 1)
  changes_mid[j] <- length(which(mid_lat[,4] != mid_lat[,j+4])) / (nrow(mid_lat) - 1)
  changes_low[j] <- length(which(low_lat[,4] != low_lat[,j+4])) / (nrow(low_lat) - 1)
  mega_high[j] <- length(which(high_lat_m[,4] != high_lat_m[,j+4])) / (nrow(high_lat) - 1)
  mega_mid[j] <- length(which(mid_lat_m[,4] != mid_lat_m[,j+4])) / (nrow(mid_lat) - 1)
  mega_low[j] <- length(which(low_lat_m[,4] != low_lat_m[,j+4])) / (nrow(low_lat) - 1)
  changes_high_a[j] <- sum(high_lat[which(high_lat[,4] != high_lat[,j+4]), 3]) / high_lat_area
  changes_mid_a[j] <- sum(mid_lat[which(mid_lat[,4] != mid_lat[,j+4]), 3]) / mid_lat_area
  changes_low_a[j] <- sum(low_lat[which(low_lat[,4] != low_lat[,j+4]), 3]) / low_lat_area
  mega_high_a[j] <- sum(high_lat_m[which(high_lat_m[,4] != high_lat_m[,j+4]), 3]) / high_lat_area
  mega_mid_a[j] <- sum(mid_lat_m[which(mid_lat_m[,4] != mid_lat_m[,j+4]), 3]) / mid_lat_area
  mega_low_a[j] <- sum(low_lat_m[which(low_lat_m[,4] != low_lat_m[,j+4]), 3]) / low_lat_area
}
